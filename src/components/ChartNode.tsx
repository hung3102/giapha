...
const ChartNode = ({
  ...
  const couple = datasource.name.split("\n");

  const nodeEles = [<div className="husband">{couple[0]}</div>];

  couple.shift();
  couple.forEach((wife: string) => {
    nodeEles.push(<div className="wife">{wife}</div>);
  });
    ...
    <li className="oc-hierarchy">
      ...
      >
        ...
        {datasource.info && (
          <>
            <i
              className="oci oci-info-circle second-menu-icon"
              onClick={() => {
